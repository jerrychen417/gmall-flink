...
//        System.setProperty("HADOOP_USER_NAME", "atguigu");
        ...
        productStatsWithTmDstream.addSink(ClickHouseUtil.getSink("insert into product_stats_210726  values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)"));
        ...
        /*//todo 2. 消费Kafka数据创建流
        String groupId = "product_stats_app_2107";
        ...
        SingleOutputStreamOperator<ProductStats> productStatsWithWMDS= unionDS.assignTimestampsAndWatermarks(WatermarkStrategy
                .<ProductStats>forBoundedOutOfOrderness(Duration.ofSeconds(2)).withTimestampAssigner(new SerializableTimestampAssigner<ProductStats>() {
                    @Override
                    public long extractTimestamp(ProductStats element, long recordTimestamp) {
                        return element.getTs();
                    }
        ...
        //todo 6.分组开窗 & 聚合
        SingleOutputStreamOperator<ProductStats> productStatsDS = productStatsWithWMDS.keyBy(productStats -> productStats.getSku_id())
                .window(TumblingEventTimeWindows.of(Time.seconds(10)))
                .reduce(new ReduceFunction<ProductStats>() {
                    @Override
                    public ProductStats reduce(ProductStats stats1, ProductStats stats2) throws Exception {
        ...
        //7.3 补充品类纬度
        SingleOutputStreamOperator<ProductStats> productStatsWithCategory3DStream = AsyncDataStream.unorderedWait(productStatsWithSpuDS,
                new DimAsyncFunction<ProductStats>("DIM_BASE_CATEGORY3") {
                    @Override
                    public String getKey(ProductStats productStats) {
                        return String.valueOf(productStats.getCategory3_id());
